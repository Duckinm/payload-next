...
const HeroCard: React.FC<Props> = ({ starred, border, backgroundColor, titleColor, descColor }) => {
  ...
  return (
    ...
    >
      <div className="w-1/4">
        <img
          src={starred.value.slider[0].image.cloudStorageUrl || "/media/" + starred.value.slider[0].image.filename}
      ...
      <div className="flex items-start w-3/4">
        <div className="w-auto px-1">
          <h3 className="text-2xl leading-none font-minimal" style={{ color: titleColor ? titleColor : "#000" }}>
            {starred.value.title ? starred.value.title : "Lorem Ipsum"}
          </h3>
          <p className="leading-none line-clamp-2 md:line-clamp-3 font-minimal" style={{ color: descColor ? descColor : "#000" }}>
...
export const Hero: React.FC<Type> = ({ card, heroImage }) => {
  return (
    <section>
      <img src={heroImage.cloudStorageUrl || "/media/" + heroImage.filename} alt={heroImage.alt} className="object-cover object-bottom w-full h-full" />
      <div className="container relative flex flex-col items-center md:flex-row space-y-1 md:space-y-0 justify-center space-x-0.5 mt-[-20px] md:mt-[-60px]">
        {card.map(({ backgroundColor, border, starred, titleColor, descColor }, key) => (
          <HeroCard key={key} starred={starred} border={border} titleColor={titleColor?.color} descColor={descColor?.color} backgroundColor={backgroundColor?.color} />
        ))}
      </div>
    </section>
  )
