...
const ContactForm = () => {
  ...
  return (
    <form id="contactForm" className="flex flex-col items-center w-full space-y-1 md:w-1/2" autoComplete="off" onSubmit={handleSubmit(onSubmit)}>
      <label htmlFor="message" hidden></label>
      <textarea
        placeholder="Details"
        className={` mb-1
          ${touchedFields && errors && errors.message?.type === "required" ? "error" : ""}`}
      ...
      <input
        type="text"
        placeholder="Email"
        className={touchedFields && errors && errors.mail?.type === "required" ? "error" : ""}
        {...register("mail", {
          required: true,
      ...
      <input type="number" placeholder="Phone" className={touchedFields && errors && errors.tel?.type === "required" ? "error" : ""} {...register("tel", { required: true })} />
    ...
    <section id="submission" className="container my-20 lg:my-40 lg:max-w-4xl">
      <div className="flex flex-col space-y-8 md:flex-row md:space-x-8 md:space-y-0">
        <div className="w-full text-center md:w-1/2">
          <h3 className="font-cera tracking-[-4%] text-4xl lg:text-5xl text-primary mb-8">{settings?.contactInfo?.title ? settings.contactInfo.title : "Contact Request"}</h3>
