  ...
  return (
    <Menu as="div" className="relative inline-block w-12 text-left">
      ...
      >
        <Menu.Items
          as="ul"
          className="absolute right-0 z-[52] w-12 mt-1 origin-top-right bg-transparent focus:outline-none"
        >
            ...
            return (
              <Menu.Item as="li" key={locale}>
                <Link href={{ pathname, query }} as={asPath} locale={locale}>
                  <a
                    className={`flex items-center justify-center w-full p-3 text-xl leading-none tracking-wider rounded-full font-minimal text-white bg-black hover:bg-opacity-40 mb-0.5 ${
                      index % 2 ? 'bg-opacity-80' : 'bg-opacity-70'
                    }`}
                  >
                    {locale}
                  </a>
