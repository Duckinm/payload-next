  ...
  return (
    <>
      ...
      <main className="container">
        ...
        {galleries?.slider && <Slider data={galleries.slider} className="h-[205.71px] sm:h-[274.29px] md:h-[329.14px] lg:h-[438.86px] xl:h-[548.57px]" />}

        <div className={`flex items-center ${galleries?.sharesAlignment == "right" ? "justify-end" : "justify-start"}`}>
          <Shares />
        </div>
        <div className="grid gap-3 my-10 md:grid-cols-2">
          {galleries?.highlight?.highlightCard.map(({ topic, options, menu, blockType }, key) => (
            <CardInfo key={key} blockType={blockType} topic={topic} options={options} menu={menu} />
          ))}
          <ButtonGroup layout={galleries?.highlight?.layoutPlan} map={galleries?.highlight?.map} streetView={galleries?.highlight?.streetView} />
        </div>
        <div className="grid gap-3 mb-20 lg:mb-40 md:grid-cols-2">
          {galleries?.components?.map(({ topic, options, menu, blockType }, key) => (
            <CardInfo key={key} blockType={blockType} topic={topic} options={options} menu={menu} />
          ))}
        </div>
      </main>
...
export const getStaticProps: GetStaticProps = async ({ locale, params }) => {
  const galleriesReq = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/galleries?locale=${locale}&where[slug][equals]=${params?.slug}`)
