      ...
      <a className="flex flex-col md:flex-row bg-grey-100 rounded-[20px] md:h-[300px] shadow-lg hover:shadow-2xl hover:scale-[1.02] transition">
        <div className="w-full md:w-1/3">
          {cover ? (
            ...
            <img src="https://via.placeholder.com/700x500" alt="fake" className="rounded-tl-[12px] rounded-bl-[12px] object-cover w-full h-full max-h-full border-tertiary-variants" />
          )}
        </div>
        <div className="flex w-full p-5 md:w-2/3">
          <div className="w-auto pr-3 md:pr-8">
            <h2 className="mb-2 text-3xl font-bold leading-none text-black lg:text-4xl md:mb-4 font-minimal">{title}</h2>
...
export const getStaticProps: GetStaticProps = async ({ locale }) => {
  const galleries = await fetch(`${process.env.NEXT_PUBLIC_SERVER_URL}/api/galleries/?locale=${locale}`).then((res) => res.json())
